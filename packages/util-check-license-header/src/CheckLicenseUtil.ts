...
function collectFiles({ dirPath, excludes, extensions, collectedFiles }) {
  if (excludes.some((exclude) => minimatch(dirPath, exclude))) return;
...
function hasLicenseHeader(fileHeaderLines, licenseHeaderLines) {
  for (let i = 0; i < licenseHeaderLines.length - 1; i++) {
    if (
      !fileHeaderLines[i + 1] ||
      fileHeaderLines[i + 1].indexOf(licenseHeaderLines[i]) === -1
    ) {
      return false;
    }
  }
...
function checkLicenseAndCopyright({
  ...
  console.log(
    `${AnsiColorEnum.CYAN}\n\n${INITIAL_MESSAGE}${AnsiColorEnum.RESET}\n\n`
  );

  if (!paths || !year || !author || !extensions) {
    console.error(
      `${AnsiColorEnum.RED}${MISSING_PARAMS_MESSAGE}${AnsiColorEnum.RESET}`
    );
    process.exit(1);
  }
  ...
  const licenseHeader = LICENSE_HEADER_TEMPLATE.replace(
    /{year}/gi,
    year
  ...
  for (const basePath of pathList) {
    ...
    for (const file of files) {
      ...
      if (!hasLicense || !hasCopyright) {
        const output = `${AnsiColorEnum.BOLD}${AnsiColorEnum.LIGHTBLUE}${file}${AnsiColorEnum.RESET}${copyrightMsg} ${licenseMsg}`;
        console.log(output);
      }
    }
  }
...
export { checkLicenseAndCopyright };
