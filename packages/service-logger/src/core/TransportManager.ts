...
import { LogLevelNumberEnum } from '../enums/LogLevelNumberEnum';
...
export class LogTransportManager implements IlogTransport {
  ...
  shouldLog(transportLevel: string, configLevel: string): boolean {
    ...
    const desiredLevel =
      LogLevelNumberEnum[(configLevel || this.config.level).toUpperCase()];
  ...
  log(level: string, message: string): void {
    for (let i = 0; i < this.transports.length; i++) {
      const logMessageBuilder = new LogMessageBuilder(
        this.config,
        this.transports[i]
      );
