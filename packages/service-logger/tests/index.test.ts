...
import { LogLevelEnum } from '../src/enums/LogLevelEnum';
...
describe('End-to-End Tests for Logger Module', () => {
  ...
  it('should check for prefix and datetime in log messages, if passed', () => {
    ...
    expect(spyDebug).toHaveBeenCalledWith(
      expect.stringContaining('Test Prefix 123456789 This is a debug message')
    );
    expect(spyInfo).toHaveBeenCalledWith(
      expect.stringContaining('Test Prefix 123456789 This is an info message')
    );
    expect(spyWarn).toHaveBeenCalledWith(
      expect.stringContaining('Test Prefix 123456789 This is a warn message')
    );
    // expect(spyTrace).toHaveBeenCalledWith(expect.stringContaining('This is a trace message'));
    expect(spyError).toHaveBeenCalledWith(
      expect.stringContaining('Test Prefix 123456789 This is an error message')
    );
  });
  ...
  it('should handle multiple transports', () => {
    ...
    expect(spyConsoleCount).toHaveBeenCalledWith(
      expect.stringContaining('This is a debug message')
    );
    expect(spyConsoleCount).toHaveBeenCalledWith(
      expect.stringContaining('This is an info message')
