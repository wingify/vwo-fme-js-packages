...
let fsUtil = {
  ...
  getAllFiles: function ({ path, excludes, extensions, allFiles }) {
    if (excludes.some((key) => path.includes(key))) {
      return;
...
let checkLicenseUtil = {
  ...
  checkLicense: function (lines, licenseHeaderLines) {
    ...
    for (let i = 0; i < licenseHeaderLines.length - 1; i++) {
      ...
      let isContainedInLicenseHEader =
        lines[i + 1].indexOf(licenseHeaderLines[i]) > -1;
  ...
  checkLicenseAndCopyright: function ({
    ...
    if (!paths || !year || !author || !extensions) {
      console.error(
        `${AnsiColorEnum.RED}${missingParamsMessage}${AnsiColorEnum.RESET}`
      );
      process.exit(1);
    }
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      for (let j = 0; j < list.length; j++) {
        let headerLines = checkLicenseUtil.readHeaderLines(
          list[j],
          stoppingCriteria
        );
        let hasCopyright;
        ...
        hasLicense = checkLicenseUtil.checkLicense(
          headerLines,
          licenseHeaderLines
        );
        if (!hasLicense) {
        ...
        isLicenseAndCopyrightPresentInAllFiles =
          isLicenseAndCopyrightPresentInAllFiles && hasCopyright && hasLicense;
